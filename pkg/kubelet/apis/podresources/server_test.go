...
func TestAvailableResourcess(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		expectedAvailableResourcesResponse *podresourcesapi.AvailableResourcesResponse
	}{
		{
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			resp, err := server.GetAvailableResources(context.TODO(), &podresourcesapi.AvailableResourcesRequest{})
			if err != nil {
				t.Errorf("want err = %v, got %q", nil, err)
			}
			if tc.expectedAvailableResourcesResponse.String() != resp.String() {
				t.Errorf("want resp = %s, got %s", tc.expectedAvailableResourcesResponse.String(), resp.String())
			}
		})
	}
}
