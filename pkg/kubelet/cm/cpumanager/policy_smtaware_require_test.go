...
type smtAwareRequirePolicyTest struct {
	description     string
...
func TestSMTAwareRequirePolicyName(t *testing.T) {
	policy, _ := NewSMTAwareRequirePolicy(topoSingleSocketHT, 1, cpuset.NewCPUSet(), topologymanager.NewFakeManager())

	policyName := policy.Name()
	if policyName != "static" {
		t.Errorf("SMTAwareRequirePolicy Name() error. expected: static, returned: %v",
			policyName)
	}
}

func TestSMTAwareRequirePolicyStart(t *testing.T) {
	testCases := []smtAwareRequirePolicyTest{
		{
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			p, _ := NewSMTAwareRequirePolicy(testCase.topo, testCase.numReservedCPUs, cpuset.NewCPUSet(), topologymanager.NewFakeManager())
			policy := p.(*smtAwareRequirePolicy)
			st := &mockState{
			...
			if !reflect.DeepEqual(err, testCase.expErr) {
				t.Errorf("SMTAwareRequirePolicy Start() error (%v). expected error: %v but got: %v",
			...
			if !testCase.stDefaultCPUSet.IsEmpty() {
				for cpuid := 1; cpuid < policy.topology.NumCPUs; cpuid++ {
					if !st.defaultCPUSet.Contains(cpuid) {
						t.Errorf("SMTAwareRequirePolicy Start() error. expected cpuid %d to be present in defaultCPUSet", cpuid)
					}
				}
			}
...
func TestSMTAwareRequirePolicyAdd(t *testing.T) {
	...
	for _, testCase := range testCases {
		policy, _ := NewSMTAwareRequirePolicy(testCase.topo, testCase.numReservedCPUs, cpuset.NewCPUSet(), topologymanager.NewFakeManager())
		...
		if !reflect.DeepEqual(err, testCase.expErr) {
			t.Errorf("SMTAwareRequirePolicy Allocate() error (%v). expected add error: %v but got: %v",
				testCase.description, testCase.expErr, err)
		}
		...
		if testCase.expCPUAlloc {
			...
			if !found {
				t.Errorf("SMTAwareRequirePolicy Allocate() error (%v). expected container %v to be present in assignments %v",
					testCase.description, container.Name, st.assignments)
			}

			if !reflect.DeepEqual(cset, testCase.expCSet) {
				t.Errorf("SMTAwareRequirePolicy Allocate() error (%v). expected cpuset %v but got %v",
					testCase.description, testCase.expCSet, cset)
			}

			if !cset.Intersection(st.defaultCPUSet).IsEmpty() {
				t.Errorf("SMTAwareRequirePolicy Allocate() error (%v). expected cpuset %v to be disoint from the shared cpuset %v",
		...
		if !testCase.expCPUAlloc {
			...
			if found {
				t.Errorf("SMTAwareRequirePolicy Allocate() error (%v). Did not expect container %v to be present in assignments %v",
					testCase.description, container.Name, st.assignments)
			}
		}
	}
...
func TestSMTAwareRequirePolicyRemove(t *testing.T) {
	testCases := []smtAwareRequirePolicyTest{
		{
	...
	for _, testCase := range testCases {
		...
		if !reflect.DeepEqual(st.defaultCPUSet, testCase.expCSet) {
			t.Errorf("SMTAwareRequirePolicy RemoveContainer() error (%v). expected default cpuset %v but got %v",
		...
		if _, found := st.assignments[testCase.podUID][testCase.containerName]; found {
			t.Errorf("SMTAwareRequirePolicy RemoveContainer() error (%v). expected (pod %v, container %v) not be in assignments %v",
...
type smtAwareRequirePolicyTestWithResvList struct {
	description     string
	topo            *topology.CPUTopology
	numReservedCPUs int
...
func TestSMTAwareRequirePolicyStartWithResvList(t *testing.T) {
	testCases := []smtAwareRequirePolicyTestWithResvList{
		{
			description:     "empty cpuset",
			topo:            topoDualSocketHT,
			numReservedCPUs: 2,
...
func TestSMTAwareRequireAddWithResvList(t *testing.T) {
	...
	for _, testCase := range testCases {
		policy, _ := NewSMTAwareRequirePolicy(testCase.topo, testCase.numReservedCPUs, testCase.reserved, topologymanager.NewFakeManager())
