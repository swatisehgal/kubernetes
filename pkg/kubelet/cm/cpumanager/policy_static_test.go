...
func TestStaticPolicyName(t *testing.T) {
	...
	policy, _ := NewStaticPolicy(topoSingleSocketHT, 1, cpuset.New(), topologymanager.NewFakeManager(tCtx), nil)
...
func TestStaticPolicyStart(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			...
			p, _ := NewStaticPolicy(testCase.topo, testCase.numReservedCPUs, cpuset.New(), topologymanager.NewFakeManager(tCtx), testCase.options)
...
func TestTopologyAwareAllocateCPUs(t *testing.T) {
	...
	for _, tc := range testCases {
		p, _ := NewStaticPolicy(tc.topo, 0, cpuset.New(), topologymanager.NewFakeManager(tCtx), nil)
		policy := p.(*staticPolicy)
		st := &mockState{
...
func TestStaticPolicyAddWithResvList(t *testing.T) {
	...
	tCtx := ktesting.Init(t)
	for _, testCase := range testCases {
		policy, _ := NewStaticPolicy(testCase.topo, testCase.numReservedCPUs, testCase.reserved, topologymanager.NewFakeManager(tCtx), nil)
