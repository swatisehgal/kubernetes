...
// SMTAwareRequire is the name of the static policy
const PolicySMTAwareRequire policyName = "smtaware-require"

// smtAwareRequirePolicy is a CPU manager policy that does not change CPU
...
// - The CPU request is am even positive integer.
//
...
// reconcile period.
type smtAwareRequirePolicy struct {
	// cpu socket topology
...
// Ensure smtAwareRequirePolicy implements Policy interface
var _ Policy = &smtAwareRequirePolicy{}

// NewSMTAwareRequire returns a CPU manager policy that does not change CPU
// assignments for exclusively pinned guaranteed containers after the main
// container process starts.
...
func (p *smtAwareRequirePolicy) Name() string {
	return string(PolicySMTAwareRequire)
}
...
func (p *smtAwareRequirePolicy) validateState(s state.State) error {
...
func (p *smtAwareRequirePolicy) allocateCPUs(s state.State, numCPUs int, numaAffinity bitmask.BitMask, reusableCPUs cpuset.CPUSet) (cpuset.CPUSet, error) {
