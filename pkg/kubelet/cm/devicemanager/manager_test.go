...
func TestPodContainerDeviceToAllocate(t *testing.T) {
	...
	testManager.podDevices.insert("pod3", "con3", resourceName2,
		...
		constructAllocResp(map[string]string{"/dev/r1dev5": "/dev/r1dev5"},
			map[string]string{"/home/r1lib1": "/usr/r1lib1"}, map[string]string{}))
	testManager.podDevices.insert("pod4", "con4", resourceName4,
		checkpoint.DevicesPerNUMA{nodeWithoutTopology: []string{"dev6", "dev7"}},
		constructAllocResp(map[string]string{"/dev/r4dev6": "/dev/r4dev6", "/dev/r4dev7": "/dev/r4dev7"},
			map[string]string{"/home/r2lib1": "/usr/r2lib1"}, map[string]string{}))
	testManager.podDevices.insert("pod4", "con4", resourceName4,
		checkpoint.DevicesPerNUMA{nodeWithoutTopology: []string{"dev6"}},
		constructAllocResp(map[string]string{"/dev/r4dev6": "/dev/r4dev6"},
			map[string]string{"/home/r2lib1": "/usr/r2lib1"}, map[string]string{}))
	testManager.podDevices.insert("pod5", "con5", resourceName4,
		checkpoint.DevicesPerNUMA{nodeWithoutTopology: []string{"dev8", "dev9"}},
		constructAllocResp(map[string]string{"/dev/r4dev8": "/dev/r4dev8", "/dev/r4dev9": "/dev/r4dev9"},
			map[string]string{"/home/r2lib1": "/usr/r2lib1"}, map[string]string{}))
	...
	// testManager.healthyDevices[resourceName1].Insert("dev1")
	// testManager.healthyDevices[resourceName1].Insert("dev2")
	testManager.healthyDevices[resourceName4] = sets.NewString()
	testManager.healthyDevices[resourceName4].Insert("dev8")
	testManager.healthyDevices[resourceName4].Insert("dev9")

	// testManager.healthyDevices[resourceName4].Insert("dev9")

	// testManager.allocatedDevices[resourceName1] = sets.NewString()
	// testManager.allocatedDevices[resourceName1].Insert("dev1")
	// testManager.allocatedDevices[resourceName1].Insert("dev2")

	// err = testManager.writeCheckpoint()

	expectedPodDevices := testManager.podDevices
	t.Logf("expectedPodDevices:%v", expectedPodDevices)
	expectedAllocatedDevices := testManager.podDevices.devices()
	t.Logf("expectedAllocatedDevices:%v", expectedAllocatedDevices)
	expectedAllDevices := testManager.healthyDevices
	t.Logf("expectedAllDevices:%v", expectedAllDevices)
	...
	for podUID, containerDevices := range expectedPodDevices.devs {
		for conName, resources := range containerDevices {
			for resource := range resources {
				expDevices := expectedPodDevices.containerDevices(podUID, conName, resource)
				t.Logf("expDevices:%v", expDevices)
				testDevices := testManager.podDevices.containerDevices(podUID, conName, resource)
				t.Logf("testDevices:%v", testDevices)
				as.True(reflect.DeepEqual(expDevices, testDevices))
				opts1 := expectedPodDevices.deviceRunContainerOptions(podUID, conName)
				opts2 := testManager.podDevices.deviceRunContainerOptions(podUID, conName)
				as.Equal(len(opts1.Envs), len(opts2.Envs))
	...
	// testManager.allocatedDevices[resourceName4] = sets.NewString()
	// testManager.allocatedDevices[resourceName4].Insert("dev6", "dev7")
	...
	testCases := []struct {
	...
	}{
		{
			...
			expErr:                   fmt.Errorf("not enough healthy devices; can't allocate unhealthy devices %s", resourceName1),
		},
		{
		...
		},
		{
			...
			reusableDevices:          sets.NewString(),
			expectedAllocatedDevices: nil,
			expErr:                   fmt.Errorf("previously allocated devices are now unhealthy; can't allocate unhealthy devices %s", resourceName2),
		},
	}
