...
func (p *bestEffortPolicy) Allocate(logger klog.Logger, s state.State, pod *v1.Pod, container *v1.Container) (rerr error) {
...
func (p *bestEffortPolicy) GetPodTopologyHints(logger klog.Logger, s state.State, pod *v1.Pod) map[string][]topologymanager.TopologyHint {
...
func (p *bestEffortPolicy) GetAllocatableMemory(logger klog.Logger, s state.State) []state.Block {
	return p.static.GetAllocatableMemory(logger, s)
}
