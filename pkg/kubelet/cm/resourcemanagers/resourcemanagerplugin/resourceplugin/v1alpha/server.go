...
import (
	...
	"k8s.io/klog/v2"
	api "k8s.io/kubelet/pkg/apis/resourceplugin/v1alpha"
...
func NewServer(socketPath string, rh RegistrationHandler, ch ClientHandler) (Server, error) {
	...
	klog.V(2).InfoS("Creating resource plugin registration server", "version", api.Version, "socket", socketPath)
	s := &server{
		socketName: name,
		socketDir:  dir,
		rhandler:   rh,
...
func (s *server) Start() error {
	klog.V(2).InfoS("Starting resource plugin registration server")

	if err := os.MkdirAll(s.socketDir, 0750); err != nil {
		klog.ErrorS(err, "Failed to create the resource plugin socket directory", "directory", s.socketDir)
		return err
	}
	...
	if err := s.rhandler.CleanupPluginDirectory(s.socketDir); err != nil {
		klog.ErrorS(err, "Failed to cleanup the resource plugin directory", "directory", s.socketDir)
		return err
	}

	ln, err := net.Listen("unix", s.SocketPath())
	if err != nil {
		klog.ErrorS(err, "Failed to listen to socket while starting resource plugin registry")
		return err
	}
...
func (s *server) Stop() error {
	s.visitClients(func(r string, c Client) {
		if err := s.disconnectClient(r, c); err != nil {
			klog.InfoS("Error disconnecting resource plugin client", "resourceName", r, "err", err)
		}
	})
...
func (s *server) Register(ctx context.Context, r *api.RegisterRequest) (*api.Empty, error) {
	klog.InfoS("Got registration request from resource plugin with resource", "resourceName", r.ResourceName)
	// TODO: Capture metrics related registration
	...
	if !s.isVersionCompatibleWithPlugin(r.Version) {
		...
		klog.InfoS("Bad registration request from resource plugin with resource", "resourceName", r.ResourceName, "err", err)
	...
	if !v1helper.IsExtendedResourceName(core.ResourceName(r.ResourceName)) {
		...
		klog.InfoS("Bad registration request from resource plugin", "err", err)
		return &api.Empty{}, err
	}

	if err := s.connectClient(r.ResourceName, filepath.Join(s.socketDir, r.Endpoint)); err != nil {
		klog.InfoS("Error connecting to resource plugin client", "err", err)
		return &api.Empty{}, err
	}
