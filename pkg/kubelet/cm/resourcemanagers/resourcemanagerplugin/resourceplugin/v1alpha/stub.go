...
type Stub struct {
	...
	stop   chan interface{}
	wg     sync.WaitGroup
	update chan []*pluginapi.Resource
...
type stubGetPreferredAllocFunc func(r *pluginapi.PreferredAllocationRequest, devs map[string]pluginapi.Resource) (*pluginapi.PreferredAllocationResponse, error)

func defaultGetPreferredAllocFunc(r *pluginapi.PreferredAllocationRequest, devs map[string]pluginapi.Resource) (*pluginapi.PreferredAllocationResponse, error) {
...
type stubAllocFunc func(r *pluginapi.AllocateRequest, devs map[string]pluginapi.Resource) (*pluginapi.AllocateResponse, error)

func defaultAllocFunc(r *pluginapi.AllocateRequest, devs map[string]pluginapi.Resource) (*pluginapi.AllocateResponse, error) {
...
func NewResourcePluginStub(devs []*pluginapi.Resource, socket string, name string, preStartContainerFlag bool, getPreferredAllocationFlag bool) *Stub {
	return &Stub{
		devs:                       devs,
		socket:                     socket,
		resourceName:               name,
		...
		update: make(chan []*pluginapi.Resource),
...
// SetGetPreferredAllocFunc sets allocFunc of the resource plugin
...
// ListAndWatch lists resources and update that list according to the Update call
func (m *Stub) ListAndWatch(e *pluginapi.Empty, s pluginapi.ResourcePlugin_ListAndWatchServer) error {
	klog.InfoS("ListAndWatch")

	s.Send(&pluginapi.ListAndWatchResponse{Resources: m.devs})
	...
	for {
		...
		case updated := <-m.update:
			s.Send(&pluginapi.ListAndWatchResponse{Resources: updated})
		}
	}
}

// Update allows the resource plugin to send new resources through ListAndWatch
func (m *Stub) Update(devs []*pluginapi.Resource) {
	m.update <- devs
}

// GetPreferredAllocation gets the preferred allocation from a set of available resources
func (m *Stub) GetPreferredAllocation(ctx context.Context, r *pluginapi.PreferredAllocationRequest) (*pluginapi.PreferredAllocationResponse, error) {
	...
	devs := make(map[string]pluginapi.Resource)
