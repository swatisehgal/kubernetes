...
var _ = SIGDescribe("Device Plugin [Feature:DevicePluginProbe][NodeFeature:DevicePluginProbe][Serial]", func() {
	...
	testDevicePluginSimulateNodeRebootScenario(f, kubeletdevicepluginv1beta1.DevicePluginPath)
})
...
const (
	...
	// This is the sleep interval specified in the command executed in the pod to ensure container is not restarted
	sleepIntervaltoEnsureNoPodRestart string = "24h"

	// This is the sleep interval specified in the command executed in the pod so that container is restarted at a frequency of 60 seconds
	sleepIntervalForPodRestart string = "60s"
)
...
func testDevicePlugin(f *framework.Framework, pluginSockDir string) {
	...
	ginkgo.Context("DevicePlugin [Serial] [Disruptive]", func() {
		...
		/*
			Test Description: Keeps device plugin assignments across pod restarts (no kubelet restart, device plugin re-registration)
			Rationale: 		  The aim here is to isolate test cases that are pertaining to the container restarts. This is done by specifying
			                  sleep interval of 60s in the container command that would trigger the the container to restart every minute. We want
							  to showcase here that the device allocated to a container persists across container restarts.
			Steps:
			1. Deploy a pod: Pod1 with sleep interval of 60 seconds.
			2. Determine the device allocated to the pod by parsing logs of the pod requesting device.
			3. Ensure that the pod is allocated a device.
			4. Wait for the pod to restart. This should happen every 60 seconds as it is influenced by the sleep interval.
			6. Determine the device allocated to the pod. The pod should keep the device it was allocated prior to pod restart.
			7. Deploy another pod: Pod2 with sleep interval of 60 seconds.
			8. Determine the device allocated to the pod.
			9. Ensure that the pod is allocated a device.
		*/
		...
		/*
			Test Description: Keeps device plugin assignments across kubelet restarts (no pod restart, no device plugin re-registration)
			Rationale: 		  The aim is to explictly capture the kubelet restart scenario where the device plugin pod doesn't re-register
							  itself to Kubelet. Since the device allocation information is recovered from the checkpoint file, it expected that the
							  device will persist across kubelet restart.
							  NOTE: The sleep interval is specified to be a large value (24h) to empahsize that this is strictly a kubelet restart
							  test case and shouldn't be confused with container restart scenarios.
			Steps:
			1. Deploy Pod1 with a high sleep interval of 24h to prevent pod restart.
			2. Determine the device allocated to the pod.
			3. Ensure that the pod is allocated a device.
			4. Restart Kubelet and wait for the node to be ready for scheduling.
			5. Ensure that the node is schedulable.
			6. Since the application pod starts before the device plugin re-registers itself, the pod would fail at admission time.
			7. Determine the device allocated to the pod. The pod should keep the device it was allocated prior to kubelet restart.
		*/
		ginkgo.It("Keeps device plugin assignments across kubelet restarts (no pod restart, no device plugin re-registration)", func(ctx context.Context) {
			podRECMD := fmt.Sprintf("devs=$(ls /tmp/ | egrep '^Dev-[0-9]+$') && echo stub devices: $devs && sleep %s", sleepIntervaltoEnsureNoPodRestart)
			pod1 := e2epod.NewPodClient(f).CreateSync(ctx, makeBusyboxPod(SampleDeviceResourceName, podRECMD))
			deviceIDRE := "stub devices: (Dev-[0-9]+)"
			devID1, err := parseLog(ctx, f, pod1.Name, pod1.Name, deviceIDRE)
			framework.ExpectNoError(err, "getting logs for pod %q", pod1.Name)
		...
		*/
		...
		/*
			Test Description: Keeps device plugin assignments after kubelet restart and device plugin has been re-registered (no pod restart)
			Rationale: 		  The aim is to explicitly capture the kubelet restart scenario where the device plugin pod re-registers itself
							  to Kubelet. Since the device allocation information is recovered from the checkpoint file, it expected that the
							  device will persist across kubelet restart.
							  NOTE: The sleep interval is specified to be a large value (24h) to emphasize that this is strictly a kubelet restart
							  test case and shouldn't be confused with container restart scenarios.
			Steps:
			1. Deploy Pod1 with a high sleep interval of 24h to prevent pod restart.
			2. Determine the device allocated to the pod.
			3. Ensure that the pod is allocated a device.
			4. Restart Kubelet and wait for the node to be ready for scheduling.
			5. Since the application pod starts before the device plugin re-registers itself, the pod would fail at admission time.
			6. Re-register device plugin.
			7. Determine the device allocated to the pod. The pod should keep the device it was allocated prior to kubelet restart.
			8. Ensure that the node capacity is updated with the expected quantity of resources.
			9. Deploy another pod : Pod2 with a high sleep interval of 24h to prevent pod restart.
			10. Determine the device allocated to the pod.
			11. After the device plugin has re-registered, the list healthy devices is repopulated based on the devices discovered.
			   Since Pod1 failed with admission error, the device allocated to Pod1 is also available to be allocated to other pods.
			   So, once Pod2 is running we determine the device that was allocated it. As long as the device allocation succeeds the
			   test should pass.
		*/
...
func testDevicePluginSimulateNodeRebootScenario(f *framework.Framework, pluginSockDir string) {
	pluginSockDir = filepath.Join(pluginSockDir) + "/"
	ginkgo.Context("DevicePlugin [Serial] [Disruptive]", func() {
		...
		/*
			Test Description: Simulate node reboot scenarios where application pod appears before device plugin pod re-registers to Kubelet and fails at admission (no pod restart, no device plugin re-registration)
			Rationale: 		  The aim is to simulate node reboot scenario by removing pods using CRI before kubelet is started. In addition to that,
							  ...
							  admission time and therefore the device allocation does not persist across node reboot.
							  NOTE: The sleep interval is specified to be a large value (24h) to emphasize that this is strictly a kubelet restart
							  test case and shouldn't be confused with container restart scenarios.
			Steps:
			1. Deploy Pod1 with a high sleep interval of 24h to prevent pod restart.
			...
			6. Restart Kubelet and wait for the node to be ready for scheduling.
			7. Ensure that the node is schedulable.
			8. Since the application pod starts before the device plugin re-registers itself, the pod would fail at admission time.
			9. Determine the device allocated to the pod. The pod should keep the device it was allocated prior to kubelet restart.
		*/
		ginkgo.It("Keeps device plugin assignments across kubelet restarts (no pod restart, no device plugin re-registration)", func(ctx context.Context) {
			podRECMD := fmt.Sprintf("devs=$(ls /tmp/ | egrep '^Dev-[0-9]+$') && echo stub devices: $devs && sleep %s", sleepIntervaltoEnsureNoPodRestart)
			pod1 := e2epod.NewPodClient(f).CreateSync(ctx, makeBusyboxPod(SampleDeviceResourceName, podRECMD))
